    ...
    if(self)
    {
        if([decoder respondsToSelector:@selector(allowsKeyedCoding)])
        {
            if (cityName = [decoder decodeObjectForKey:@"name"])
				...
				cityName = @"d";
			if (weather_attributes = [decoder decodeObjectForKey:@"weather"])
				[weather_attributes retain];
			else
				weather_attributes = [[NSMutableArray alloc] init];
			if (forecast_attributes = [decoder decodeObjectForKey:@"forecast"])
            ...
            if (cityAndInfoCodes = [decoder decodeObjectForKey:@"infoAndCodes"])
...
- (NSDictionary *)dictionaryForProperty:(NSString *)prop;
{
    ...
    if(obj = [self dictFromEnum:[weather_attributes objectEnumerator] forProperty:prop])
        return obj;
        
    if(obj = [self dictFromEnum:[forecast_attributes objectEnumerator] forProperty:prop])
        return obj;
